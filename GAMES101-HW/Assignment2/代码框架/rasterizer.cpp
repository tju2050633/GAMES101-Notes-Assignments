...
// 加载顶点颜色信息，id自增，一个id对应一个三角形所有顶点的颜色信息
...
{   
    ...
    // 三角形顶点到点(x,y)的向量
    w[0] = Vector3f(x, y, 0) - _v[0];
    w[1] = Vector3f(x, y, 0) - _v[1];
    w[2] = Vector3f(x, y, 0) - _v[2];

    // 判断叉乘是否同向
    if (v[0].cross(w[0]).z() > 0 && v[1].cross(w[1]).z() > 0 && v[2].cross(w[2]).z() > 0)
        return true;
    else if (v[0].cross(w[0]).z() < 0 && v[1].cross(w[1]).z() < 0 && v[2].cross(w[2]).z() < 0)
        return true;
    else
        return false;
}
